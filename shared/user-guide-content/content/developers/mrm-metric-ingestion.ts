  ...
  blocks: [
    ...
    { type: 'heading', id: 'prerequisites', level: 2, text: 'Prerequisites' },
    {
      ...
      items: [
        ...
        {
          text: 'An **ingestion token**, created under Model risk management → Settings → Metrics feed & tokens. The token is shown once at creation and only a hash is stored — keep it in your secrets manager. Tokens are org-wide or scoped to a single model, and can be rotated or revoked at any time.',
        },
      ],
    },
    ...
    { type: 'heading', id: 'single-point', level: 2, text: 'Send a single point' },
    {
      type: 'code',
      language: 'bash',
    ...
    },
    {
      ...
      items: [
        ...
        {
          text: 'There is no fixed cap on batch size; the per-token rate limit (5000 requests per 15 minutes) is the volume guard. Keep batches modest so a validation error is easy to locate.',
        },
      ],
    },
    ...
    },
    {
      ...
      rows: [
        { status: 'ok', meaning: 'A threshold matched and the value is within it.' },
        { status: 'warn', meaning: 'The value breached a threshold with severity warn.' },
        { status: 'breach', meaning: 'The value breached a high or critical threshold.' },
        {
          status: 'no_threshold',
          meaning: 'No threshold matched this metric, segment and window. The point is stored.',
        },
        {
          status: 'duplicate',
          meaning: 'An identical point already exists. Nothing new is stored or evaluated.',
        },
      ],
    },
    {
      type: 'paragraph',
      text: 'When a threshold matched, the result carries a frozen snapshot of it: op is one of gt, gte, lt, lte or outside; scalar operators carry value_num, while outside carries value_lo and value_hi; severity is warn, high or critical. pointId is the stored point’s id, or null for duplicates.',
    },
    { type: 'heading', id: 'idempotency', level: 2, text: 'Idempotency and deduplication' },
    {
      type: 'paragraph',
    ...
    {
      type: 'paragraph',
      text: 'Errors return { message, data: { message, ... } }. The rate-limit response is flat: { message, statusCode }.',
    },
    {
      type: 'table',
      columns: [
      ...
      rows: [
        ...
        {
          code: '429',
          when: 'Token rate limit exceeded (5000 requests per 15 minutes). RateLimit headers indicate when to resume.',
    ...
    { type: 'heading', id: 'after-ingestion', level: 2, text: 'What happens after ingestion' },
    {
      type: 'paragraph',
