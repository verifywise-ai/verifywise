...
describe("useModelInventoryChangeHistory", () => {
  ...
  it("should call useEntityChangeHistory with 'model_inventory' entity type", () => {
    const mockData = {
      pages: [{
        data: [
          {
    ...
    const { result } = renderHook(() => useModelInventoryChangeHistory(1), {
      wrapper: createWrapper(),
    });

    expect(mockUseEntityChangeHistory).toHaveBeenCalledWith("model_inventory", 1);
    expect(result.current.isSuccess).toBe(true);
  });

  it("should pass undefined when modelInventoryId is not provided", () => {
    mockUseEntityChangeHistory.mockReturnValue({
      data: undefined,
      isLoading: false,
      isSuccess: false,
    ...
    renderHook(() => useModelInventoryChangeHistory(undefined), {
      wrapper: createWrapper(),
    });

    expect(mockUseEntityChangeHistory).toHaveBeenCalledWith("model_inventory", undefined);
  });
