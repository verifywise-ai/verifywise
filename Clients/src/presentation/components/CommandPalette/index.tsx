...
} from "../../../application/commands/actionHandler";
    ...
    >
      ...
      <Box sx={{ display: "flex", alignItems: "flex-start", gap: 2 }}>
        <Box sx={{ flex: 1 }}>
          ...
          >
            Search across all projects, tasks, vendors, policies, and more in your workspace. Start
            typing to find anything instantly.
          </Typography>
...
export function CommandPalette({ open, onOpenChange }: CommandPaletteProps) {
  ...
  const handleRemoveRecentSearch = useCallback(
    (e: React.MouseEvent, timestamp: number) => {
      e.stopPropagation();
