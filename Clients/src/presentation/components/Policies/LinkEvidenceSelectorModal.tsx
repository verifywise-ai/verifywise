...
const LinkEvidenceSelectorModal: React.FC<LinkEvidenceSelectorModalProps> = ({
  ...
  const getRange = useMemo(() => {
    ...
    const end = Math.min(page * rowsPerPage + rowsPerPage, visibleEvidence.length);
    return `${start} - ${end}`;
  ...
  const tableBody = useMemo(
      ...
      <TableBody>
        {visibleEvidence.length > 0 ? (
          visibleEvidence
            ...
            .map((ev) => {
              const disabled = linkedEvidenceIds.some((id) => Number(id) === Number(ev.id));
              ...
              return (
                <TableRow
                  ...
                  onClick={() => !disabled && toggleSelect(ev.id)}
                >
                  <TableCell>
                    <Box
                      sx={{
    ...
    [visibleEvidence, page, rowsPerPage, hidePagination, linkedEvidenceIds, selected, toggleSelect],
  );
            ...
            <TableFooter>
              <TableRow sx={tableFooterRowStyle(theme)}>
                <TableCell sx={showingTextCellStyleForPolicyLinked(theme)}>
                  Showing {getRange} of {visibleEvidence.length} evidence(s)
                </TableCell>
