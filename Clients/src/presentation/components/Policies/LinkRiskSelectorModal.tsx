...
const LinkRiskSelectorModal: React.FC<LinkRiskSelectorModalProps> = ({
  ...
  const handleSort = (columnId: string) => {
    ...
    localStorage.setItem(SORT_KEY, JSON.stringify({ key: columnId, direction: "asc" }));
  };
  ...
  const getRange = useMemo(() => {
    ...
    const end = Math.min(page * rowsPerPage + rowsPerPage, visibleRisks?.length ?? 0);
    return `${start} - ${end}`;
      ...
      <TableBody>
        {visibleRisks?.length ? (
          visibleRisks
            ...
            .map((risk) => {
              const disabled = linkedRiskIds.some((id) => Number(id) === Number(risk.id));
              ...
              return (
                <TableRow
                  ...
                  sx={{
                    ...
                    ...tableRowHoverStyle,
                    ...(deletingId === risk.id?.toString() && tableRowDeletingStyle),
                    opacity: disabled ? 0.45 : 1,
                    cursor: disabled ? "not-allowed" : "pointer",
                  }}
                  onClick={() => !disabled && toggleSelect(risk.id)}
                >
                  {/* RISK NAME */}
                  ...
                  {/* LEVEL */}
                  <TableCell>{risk.risk_level_autocalculated}</TableCell>

                  {/* RISK OWNER */}
                  <TableCell>{risk ? userMap.get(String(risk.risk_owner)) || "-" : "-"}</TableCell>
                  {/* Checkbox */}
                  <TableCell width={50}>
                    <Checkbox
                      disabled={disabled}
    ...
    >
      <TableContainer sx={{ maxHeight: 450, overflow: "auto" }}>
        ...
        >
          ...
          {tableBody}
          {paginated && !hidePagination && visibleRisks && visibleRisks.length > 0 && (
            <TableFooter>
              <TableRow sx={tableFooterRowStyle(theme)}>
                <TableCell sx={showingTextCellStyleForPolicyLinked(theme)}>
                  Showing {getRange} of {visibleRisks?.length} risk(s)
                </TableCell>
                <TablePagination
