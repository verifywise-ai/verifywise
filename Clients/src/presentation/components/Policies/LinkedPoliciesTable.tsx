...
const LinkedObjectsTable: React.FC<LinkedObjectsTableProps> = ({
  ...
  const mergedItems = useMemo(() => {
    return items.map((linked) => {
      ...
      // Match based on type
      if (type === "risk") {
        source = projectRisk.find((r) => Number(r.id) === Number(linked.object_id));
      }

      if (type === "evidence") {
        source = evidenceData.find((e) => Number(e.id) === Number(linked.object_id));
      }
      ...
      return {
        ...
        // ---- DUE DATE ----
        due_date: type === "risk" ? source?.deadline || "-" : type === "evidence" ? "-" : "-",
      };
    });
      ...
      <TableBody>
        {sortedData.length ? (
          ...
          <TableRow>
            <TableCell
              colSpan={
                TABLE_COLUMNS.filter((c) => !(c.id === "due_date" && type !== "risk")).length
              }
              align="center"
            >
