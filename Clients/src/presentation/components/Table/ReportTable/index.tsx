...
// Map from column label to column key for visibility filtering
              ...
              >
                ...
                {sortable && (
                  ...
                  >
                    ...
                    {sortConfig.key !== column && <ChevronsUpDown size={16} />}
                  </Box>
                )}
              </Box>
            </TableCell>
...
const ReportTable: React.FC<IReportTablePropsExtended> = ({
  ...
  const theme = useTheme();
  const [rowsPerPage, setRowsPerPage] = useState(() => getPaginationRowCount("reporting", 10));

  // Filter columns based on visibleColumns set
  const visibleColumnList = useMemo(() => {
    if (!visibleColumns || visibleColumns.size === 0) return columns;
    return columns.filter((col) => {
      const key = COLUMN_LABEL_TO_KEY[col];
      return !key || visibleColumns.has(key);
    });
  ...
  const sortedRows = useMemo(() => {
    ...
    return sortableData.sort((a: any, b: any) => {
      ...
      if (sortKey.includes("file") || sortKey.includes("name")) {
        ...
        sortKey.includes("upload") ||
        sortKey.includes("time")
      ) {
