...
type AutomationSaveField = "name" | "triggerId" | "actions" | "params";

type AutomationSaveFieldErrors = Partial<Record<AutomationSaveField, string>>;

const AUTOMATION_SAVE_FIELDS: AutomationSaveField[] = ["name", "triggerId", "actions", "params"];

function extractAutomationSaveFieldErrors(error: unknown): AutomationSaveFieldErrors {
  const fieldErrors: AutomationSaveFieldErrors = {};
  ...
  const collect = (errors: unknown) => {
    ...
    for (const entry of errors) {
      if (
        ...
        AUTOMATION_SAVE_FIELDS.includes(entry.field as AutomationSaveField)
      ) {
        fieldErrors[entry.field as AutomationSaveField] = entry.message;
      }
    }
  };

  if (error instanceof CustomException && error.status === 400) {
    collect(error.response?.data?.errors ?? error.response?.errors);
    return fieldErrors;
  }
  ...
  collect(axiosBody?.data?.errors ?? axiosBody?.errors);
  return fieldErrors;
}
...
const AutomationsPage: React.FC = () => {
  ...
  const [saveFieldErrors, setSaveFieldErrors] = useState<AutomationSaveFieldErrors>({});
  ...
  const fetchAutomations = useCallback(
    async (preserveSelection?: string, showLoading: boolean = true) => {
  ...
  const handleAddTrigger = (triggerTemplate: TriggerTemplate) => {
    setSaveFieldErrors((prev) => {
      if (!prev.triggerId && !prev.params) return prev;
      const { triggerId: _triggerId, params: _params, ...rest } = prev;
      return rest;
    });
    ...
    setAutomations((prev) =>
      prev.map((automation) => {
        ...
        const newName = isDefaultName
          ? getAutomationNameFromTrigger(triggerTemplate.type)
          : automation.name;
  ...
  const handleAddAction = (actionTemplate: ActionTemplate) => {
    setSaveFieldErrors((prev) => {
      ...
      const { actions: _actions, ...rest } = prev;
      return rest;
    });
    if (!selectedAutomationId) return;
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.subject = `${
              changeType === "Deleted"
                ? "Vendor {{vendor.name}} has been deleted"
                : "New vendor {{vendor.name}} has been added"
            }`;
            configuration.body = `A vendor has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.subject = `${
              changeType === "Deleted"
                ? "Model {{model.name}} has been deleted"
                : "New model {{model.name}} has been added"
            }`;
            configuration.body = `A model has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.subject = `${
              changeType === "Deleted"
                ...
                : 'New project "{{project.title}}" has been created'
            }`;
            configuration.body = `A project has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.body = `A task has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.subject = `${
              changeType === "Deleted"
                ? 'Risk "{{risk.name}}" has been deleted'
                : 'New risk "{{risk.name}}" has been identified'
            }`;
            configuration.body = `A risk has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.subject = `${
              changeType === "Deleted"
                ? 'Training "{{training.name}}" has been deleted'
                : 'New training "{{training.name}}" has been scheduled'
            }`;
            configuration.body = `A training has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.subject = `${
              changeType === "Deleted"
                ? 'Policy "{{policy.title}}" has been deleted'
                : 'New policy "{{policy.title}}" has been published'
            }`;
            configuration.body = `A policy has been ${
              changeType === "Deleted" ? "deleted from" : "added to"
            } the system.
...
This notification was sent on {{date_and_time}}.`;
            ...
            configuration.body = `An incident has been ${
...
This is an automated report generated by VerifyWise. The complete report is attached as a Word document.`);
  ...
  const handleUpdateConfiguration = (configuration: Record<string, unknown>) => {
    ...
    setSaveFieldErrors((prev) => {
      if (selectedItemType === "trigger" && (prev.params || prev.triggerId)) {
        const { params: _params, triggerId: _triggerId, ...rest } = prev;
        return rest;
      }
      if (selectedItemType === "action" && prev.actions) {
        const { actions: _actions, ...rest } = prev;
        return rest;
      }
      return prev;
...
This notification was sent on {{date_and_time}}.`,
  ...
  const hasMissingEmailRecipients = Boolean(
    selectedAutomation?.actions.some((action) => {
  ...
  const handleSaveAutomation = async () => {
    if (
      ...
      const nextErrors: AutomationSaveFieldErrors = {};
      if (!selectedAutomation?.name?.trim()) {
        nextErrors.name = "Name is required";
      }
      if (!selectedAutomation?.trigger) {
        nextErrors.triggerId = "A trigger is required";
      }
      if (!selectedAutomation || selectedAutomation.actions.length === 0) {
        nextErrors.actions = "At least one action is required";
      }
      setSaveFieldErrors(nextErrors);
      setToast({
        variant: "error",
        body: "Please complete the required fields before saving.",
        visible: true,
      });
      return;
    }
    ...
    if (!selectedAutomation.name.trim()) {
      setSaveFieldErrors({ name: "Name is required" });
      setToast({
        variant: "error",
        body: "Please enter an automation name before saving.",
        visible: true,
      });
      return;
    }
    ...
    for (const action of sendEmailActions) {
      ...
      if (!hasRecipients) {
        setSaveFieldErrors({
          actions: "Please add at least one recipient to the Send Email action",
        });
        setToast({
          variant: "error",
    ...
    try {
      ...
      const fieldErrors = extractAutomationSaveFieldErrors(error);
      ...
      // Toast always as fallback; prefer a concrete field message when present
      const firstFieldMessage = AUTOMATION_SAVE_FIELDS.map((field) => fieldErrors[field]).find(
        Boolean,
      );
      ...
      setToast({
        variant: "error",
        body: hasFieldErrors
          ? firstFieldMessage || "Please fix the highlighted fields and try again."
          ...
          {showConfigurationPanel && (
            ...
            >
              <ConfigurationPanel
                ...
                projects={projects}
                nameError={saveFieldErrors.name}
                triggerError={saveFieldErrors.triggerId}
                actionsError={saveFieldErrors.actions}
                paramsError={saveFieldErrors.params}
              />
            </Box>
          )}
        </Box>
