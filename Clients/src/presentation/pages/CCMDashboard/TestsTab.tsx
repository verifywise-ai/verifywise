...
const DEFAULT_QUERY = `SELECT COUNT(*) as count
FROM aws_s3_buckets
WHERE server_side_encryption IS NULL
  AND region = 'us-east-1'`;

const DEFAULT_EXPECTATION_CONFIG = JSON.stringify({ threshold: 0 }, null, 2);

const TestsTab: React.FC<TestsTabProps> = ({ onAlert }) => {
  ...
  const [formExpectationType, setFormExpectationType] = useState("");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (connectorsRes.length > 0 && !formConnectorId) {
        setFormConnectorId(String(connectorsRes[0].id));
      }
    } catch (err) {
      console.error("Error fetching control tests:", err);
      onAlert("error", "Failed to load control tests");
    } finally {
      setIsLoading(false);
    }
  }, [formConnectorId, onAlert]);
  ...
  const resetForm = () => {
    setFormName("");
    setFormDescription("");
    setFormConnectorId(connectors[0]?.id ? String(connectors[0].id) : "");
    setFormQueryTemplate("");
    setFormExpectationType("");
  ...
  const handleSave = async () => {
    if (!formName.trim() || !formConnectorId || !formQueryTemplate.trim() || !formExpectationType) {
      onAlert("error", "Name, connector, query, and expectation type are required");
      return;
    }
    ...
    try {
      ...
      onAlert("error", "Expectation config must be valid JSON");
      return;
    }
    ...
    try {
      if (editingTest) {
        ...
        onAlert("success", "Test updated");
      } else {
        await createCcmControlTest(payload);
        onAlert("success", "Test created");
      }
      ...
      onAlert("error", "Failed to save test");
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      onAlert("success", "Test deleted");
      fetchData();
    } catch (err) {
      console.error("Error deleting test:", err);
      onAlert("error", "Failed to delete test");
    } finally {
  ...
  const getConnectorStatus = (id: number) => {
    const c = connectors.find((conn) => conn.id === id);
    return c?.status || "unknown";
  };

  const connectorItems = [
    { _id: "", name: "Select a connector..." },
    ...connectors.map((c) => ({
      _id: String(c.id),
      name: `${c.name} (${c.type})`,
    })),
  ...
  const expectationItems = [
    { _id: "", name: "Select expectation type..." },
  ...
  const filteredTests = tests.filter((t) =>
    searchQuery
      ? t.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        getConnectorName(t.connector_id).toLowerCase().includes(searchQuery.toLowerCase())
      : true,
  );
  ...
  return (
    <>
      <Typography variant="body2" sx={{ color: "#475467", mb: 2 }}>
        Control Tests define automated checks that run on a schedule against your
        connectors. Each test queries your infrastructure and validates whether a
        specific control requirement is met.
      </Typography>
      <Stack
        direction="row"
        ...
        <EmptyState
          message={
            searchQuery
              ...
              : "No control tests yet. Add a test to automatically verify your controls."
          }
          icon={FlaskConical}
          showBorder
        />
              ...
              {filteredTests.map((t) => (
                ...
                >
                  ...
                  <TableCell
                    ...
                    align="right"
                  >
                    <Tooltip title={getConnectorStatus(t.connector_id) !== "active" ? "Cannot run — connector is not active" : "Run test now"}>
                      <span>
                        <IconButton
                          size="small"
                          onClick={() => handleRun(t.id)}
                          disabled={runningTestId === t.id || getConnectorStatus(t.connector_id) !== "active"}
                        >
                          {runningTestId === t.id ? (
      ...
      >
        <Stack spacing={6} sx={{ mt: 2 }}>
          <Field
            ...
            placeholder="e.g., S3 Bucket Encryption Check"
            isRequired
          />
          <Field
            id="test-description"
