...
import { TableContainer, Table, TableBody, TableRow, TableCell, Stack } from "@mui/material";
            ...
            .map((workflow) => (
              <TableRow
                ...
                sx={{
                  ...
                  ...workflowRowHover,
                  ...(archivedId === workflow.id?.toString() && workflowTableRowDeletingStyle),
                  cursor: "pointer",
                }}
              >
                ...
                {isVisible("entity_name") && (
                  <TableCell sx={bodyCellEntityStyle(cellStyle, sortConfig.key === "entity_name")}>
                    {entities.find((e) => e._id === workflow.entity)?.name}
                  </TableCell>
                )}
                {isVisible("steps") && (
                  <TableCell sx={bodyCellStepsStyle(cellStyle, sortConfig.key === "steps")}>
                    {workflow.steps?.length}
                  </TableCell>
                )}
                {isVisible("date_updated") && (
                  <TableCell sx={bodyCellDateStyle(cellStyle, sortConfig.key === "date_updated")}>
                    {workflow.date_updated
                      ? dayjs.utc(workflow.date_updated).format("YYYY-MM-DD HH:mm")
                      : "-"}
                  </TableCell>
                )}
                {isVisible("actions") && (
                  ...
                  >
                    <Stack direction="row" spacing={1}>
                      <CustomIconButton
                        id={workflow.id}
                        type="workflow"
                        onEdit={() => onEdit?.(workflow.id.toString())}
                        onDelete={() => onArchive?.(workflow.id.toString())}
                        onMouseEvent={() => {}}
