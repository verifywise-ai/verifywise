  ...
  function dynamicSubClauses(clause: ClauseStructISO) {
    ...
    const filteredSubClauses =
      statusFilter && statusFilter !== ""
        ? subClauses.filter((sc) => sc.status?.toLowerCase() === statusFilter.toLowerCase())
        : subClauses;
      ...
      <AccordionDetails sx={{ padding: 0 }}>
        {isLoading ? (
          ...
          filteredSubClauses.map(
            (subClause: Partial<SubClauseISO & SubClauseStructISO>, index: number) => (
              <Stack
                key={subClause.id}
          ...
          <Stack key={clause.id} sx={styles.container}>
            ...
            >
              <AccordionSummary sx={styles.accordionSummary}>
                ...
                />
                <Typography sx={{ paddingLeft: "2.5px", fontSize: 13 }}>{clause.title}</Typography>
              </AccordionSummary>
