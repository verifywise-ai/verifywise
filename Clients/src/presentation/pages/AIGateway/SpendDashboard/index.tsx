...
export default function SpendDashboardPage() {
      ...
      try {
        ...
        setByEndpoint(
          (endpointRes?.data?.data || []).map((d: any) => ({
            ...d,
            group_key: d.group_key || d.endpoint_name,
          })),
        );
        setByUser(
          (userRes?.data?.data || []).map((d: any) => ({
            ...d,
            group_key: d.group_key || d.user_name,
          })),
        );
  ...
  const byDay = useMemo(
    () => (data?.by_day || data?.byDay || []).map((d: any) => ({ ...d, day: d.day || d.period })),
    [data],
  );
  const byModel = useMemo(
    () =>
      (data?.by_model || data?.byModel || []).map((d: any) => ({
        ...d,
        group_key: d.group_key || d.model,
      })),
    [data],
  );
  const byProvider = useMemo(
    () =>
      (data?.by_provider || data?.byProvider || []).map((d: any) => ({
        ...d,
        group_key: d.group_key || d.provider,
      })),
    [data],
  );
  const errorRateByDay = useMemo(
    () => data?.error_rate_by_day || data?.errorRateByDay || [],
    [data],
  );
  const tokensPerEndpoint = useMemo(
    () =>
      (data?.tokens_per_endpoint || data?.tokensPerEndpoint || []).map((d: any) => ({
        ...d,
      ...
      >
        <Box
          sx={{
            ...
            borderRadius: "4px",
            overflow: "hidden",
            border: `1.5px dashed ${palette.border.dark}`,
          }}
        >
          ...
          {cacheStats?.total_entries > 0 && (
            <>
              <StatCard
                ...
                Icon={Zap}
                tooltip="Percentage of requests served from cache — higher means more cost savings"
              />
              <StatCard
                title="Cost saved"
                value={`$${Number(cacheStats.total_cost_saved || 0).toFixed(2)}`}
                Icon={BadgeDollarSign}
                tooltip="Money saved by serving cached responses instead of calling the LLM"
              />
            </>
          )}
        </Box>
          ...
          <Stack gap="12px">
            <Stack direction="row" alignItems="center" gap="6px">
              ...
              <MuiTooltip
                title={
                  period === "1d"
                    ? "Hourly spend breakdown for today"
                    : "Daily spend trend across all endpoints for the selected period"
                }
                arrow
                placement="top"
              >
                      ...
                      .map((m: any) => ({
                        ...
                        cost_per_req:
                          Number(m.total_requests) > 0
                            ? Number(m.total_cost) / Number(m.total_requests)
                            : 0,
                      }))
                      .sort((a: any, b: any) => b.cost_per_req - a.cost_per_req);
                    const maxCpr = Math.max(
                      ...modelsWithCpr.map((m: any) => m.cost_per_req),
                      0.000001,
                    );
                        ...
                        >
                          <Box
                            sx={{
                              ...
                              width: `${pct}%`,
                              backgroundColor:
                                i === modelsWithCpr.length - 1
                                  ? `${palette.brand.primary}15`
                  ...
                  <AreaChart data={errorRateByDay}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value, name) => {
                        ...
                        return [
                          n === "error_rate" ? `${v}%` : v,
                          n === "error_rate" ? "Error rate" : "Errors",
                        ];
                      }}
                    />
                    <Area
                      type="monotone"
                      dataKey="error_rate"
                      stroke={GUARDRAIL_ACTION_COLORS.blocked}
                  ...
                  <Stack gap="6px" flex={1}>
                    {(() => {
                      ...
                      return byProvider.map((p: any, i: number) => {
                        const pct =
                          totalProviderCost > 0
                            ? Math.round((Number(p.total_cost) / totalProviderCost) * 100)
                            : 0;
                        return (
                          <Stack
                            key={p.group_key}
                            ...
                            <Stack direction="row" alignItems="center" gap="8px">
                              <Box
                                sx={{
                                  ...
                                  height: 8,
                                  borderRadius: "50%",
                                  backgroundColor: getProviderColor(p.group_key, i),
                                  flexShrink: 0,
                                }}
                              />
                  ...
                  <Stack direction="row" alignItems="center" gap="4px">
                    <Box
                      sx={{
                        ...
                        height: 8,
                        borderRadius: "50%",
                        backgroundColor: GUARDRAIL_ACTION_COLORS.blocked,
                      }}
                    />
                    <Typography sx={{ fontSize: 11, color: palette.text.tertiary }}>
                      Blocked
                  ...
                  <Stack direction="row" alignItems="center" gap="4px">
                    <Box
                      sx={{
                        ...
                        height: 8,
                        borderRadius: "50%",
                        backgroundColor: GUARDRAIL_ACTION_COLORS.masked,
                      }}
                    />
                    <Typography sx={{ fontSize: 11, color: palette.text.tertiary }}>
                      Masked
                ...
                >
                  <AreaChart data={guardrailStats?.byDay || []} margin={{ left: 0, right: 0 }}>
                    ...
                    <Area
                      ...
                      dataKey="masked"
                      stackId="1"
                      stroke={GUARDRAIL_ACTION_COLORS.masked}
                      fill="url(#maskedGradient)"
                      strokeWidth={1.5}
                    />
                  </AreaChart>
                </ResponsiveContainer>
                            ...
                            sx={{
                              ...
                              backgroundColor:
                                d.action_taken === "blocked"
                                  ? GUARDRAIL_ACTION_COLORS.blocked
                                  : GUARDRAIL_ACTION_COLORS.masked,
                              flexShrink: 0,
                            }}
                          />
                    ...
                    (guardrailStats?.byEndpoint || []).slice(0, 6).map((ep: any, _i: number) => (
                      ...
                      >
                        ...
                        <Stack direction="row" gap="8px" alignItems="center">
                          ...
                          {ep.masked > 0 && (
                            <Typography
                              sx={{ fontSize: 11, color: GUARDRAIL_ACTION_COLORS.masked }}
                            >
                              {ep.masked} masked
                            </Typography>
                          )}
