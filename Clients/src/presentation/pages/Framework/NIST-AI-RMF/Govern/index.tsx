...
interface NISTAIRMFGovernProps {
  project: Project;
...
const NISTAIRMFGovern = ({
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        const govern = response?.data?.govern;
        if (govern) setProgress({ total: govern.total || 0, done: govern.done || 0 });
      } catch (error) {
        console.error("Error fetching NIST AI RMF Govern progress:", error);
      }
    };
    fetchProgress();
  }, [refreshTrigger]);
  ...
  useEffect(() => {
    if (expanded !== false && !subcategoriesMap[expanded]) {
      ...
      if (category) {
        fetchSubcategories(expanded, "GOVERN");
      }
    }
  ...
  const handleAccordionChange =
    (panel: number) => async (_: React.SyntheticEvent, isExpanded: boolean) => {
      ...
      if (isExpanded) {
        ...
        searchParams.set("functionId", "GOVERN");
