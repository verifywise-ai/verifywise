...
import NISTAIRMFMeasure from "../index";
...
describe("NISTAIRMFMeasure", () => {
  ...
  it("renders the title and categories", async () => {
    renderWithProviders(<NISTAIRMFMeasure {...defaultProps} />);
    expect(screen.getByText("NIST AI RMF - Measure categories")).toBeInTheDocument();
    await waitFor(() => {
  ...
  it("does not render filter bar when handlers missing", () => {
    renderWithProviders(<NISTAIRMFMeasure project={defaultProps.project} projectFrameworkId={1} />);
  ...
  it("filters categories by search term", async () => {
    renderWithProviders(
      <NISTAIRMFMeasure {...defaultProps} searchTerm="Accountability Structures" />,
    );
    await waitFor(() => {
      expect(screen.getByText(/Accountability Structures/)).toBeInTheDocument();
    });
  ...
  it("shows no matching subcategories message when filter matches nothing", async () => {
    const user = userEvent.setup();
    renderWithProviders(<NISTAIRMFMeasure {...defaultProps} statusFilter="Implemented" />);
    await waitFor(() => {
      expect(screen.getByText(/Policies and Procedures/)).toBeInTheDocument();
    });
    await user.click(screen.getByText(/Policies and Procedures/));
  ...
  it("auto-opens drawer when initial category/subcategory ids provided", async () => {
    renderWithProviders(
      <NISTAIRMFMeasure {...defaultProps} initialCategoryId="1" initialSubcategoryId="101" />,
    );
    await waitFor(() => {
      expect(screen.getByTestId("nist-drawer")).toBeInTheDocument();
    });
