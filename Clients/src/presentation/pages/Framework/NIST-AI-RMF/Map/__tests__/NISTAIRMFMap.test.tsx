...
import NISTAIRMFMap from "../index";
...
describe("NISTAIRMFMap", () => {
  ...
  it("renders the title and categories", async () => {
    renderWithProviders(<NISTAIRMFMap {...defaultProps} />);
    expect(screen.getByText("NIST AI RMF - Map categories")).toBeInTheDocument();
    await waitFor(() => {
  ...
  it("shows no matching subcategories message when filter matches nothing", async () => {
    const user = userEvent.setup();
    renderWithProviders(<NISTAIRMFMap {...defaultProps} statusFilter="Implemented" />);
    await waitFor(() => {
      expect(screen.getByText(/Policies and Procedures/)).toBeInTheDocument();
    });
    await user.click(screen.getByText(/Policies and Procedures/));
