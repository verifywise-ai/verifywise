...
const NISTAIRMFMap = ({
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        if (map) setProgress({ total: map.total || 0, done: map.done || 0 });
      } catch (error) {
        console.error("Error fetching NIST AI RMF Map progress:", error);
      }
    };
    fetchProgress();
  }, [refreshTrigger]);
  ...
  useEffect(() => {
    if (expanded !== false && !subcategoriesMap[expanded]) {
      ...
      if (category) {
        fetchSubcategories(expanded, "MAP");
      }
    }
  ...
  const handleAccordionChange =
    (panel: number) => async (_: React.SyntheticEvent, isExpanded: boolean) => {
      ...
      if (isExpanded) {
        ...
        searchParams.set("functionId", "MAP");
