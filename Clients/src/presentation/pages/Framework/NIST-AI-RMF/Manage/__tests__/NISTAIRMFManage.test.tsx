...
import NISTAIRMFManage from "../index";
...
describe("NISTAIRMFManage", () => {
  ...
  it("renders the title and categories", async () => {
    renderWithProviders(<NISTAIRMFManage {...defaultProps} />);
    expect(screen.getByText("NIST AI RMF - Manage categories")).toBeInTheDocument();
    await waitFor(() => {
  ...
  it("filters categories by search term", async () => {
    renderWithProviders(
      <NISTAIRMFManage {...defaultProps} searchTerm="Accountability Structures" />,
    );
    await waitFor(() => {
      expect(screen.getByText(/Accountability Structures/)).toBeInTheDocument();
    });
  ...
  it("shows no matching subcategories message when filter matches nothing", async () => {
    const user = userEvent.setup();
    renderWithProviders(<NISTAIRMFManage {...defaultProps} statusFilter="Implemented" />);
    await waitFor(() => {
      expect(screen.getByText(/Policies and Procedures/)).toBeInTheDocument();
    });
    await user.click(screen.getByText(/Policies and Procedures/));
