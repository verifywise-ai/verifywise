...
export default function ProjectsList() {
  ...
  const getFieldValue = useCallback((project: DeepEvalProject, field: string): string => {
    switch (field) {
      case "name":
        return project.name;
      case "useCase":
  ...
  const filteredProjects = useMemo(() => {
    ...
    return afterFilter.filter((p) =>
      [p.name, p.description, p.useCase].filter(Boolean).join(" ").toLowerCase().includes(q),
    );
      ...
      const statsArray = await Promise.all(
        (list || []).map(async (p) => {
          try {
            ...
            const total = Array.isArray(res?.experiments)
  ...
  const sortComparator = useCallback(
    (a: DeepEvalProject, b: DeepEvalProject, key: string): number => {
      switch (key) {
        ...
        case "useCase":
          return (a.useCase || "chatbot")
            .toLowerCase()
            .localeCompare((b.useCase || "chatbot").toLowerCase());
        case "runs":
  ...
  return (
    <Stack sx={{ width: "100%" }}>
      ...
      >
        {canEditProject && <MenuItem onClick={handleMenuEdit}>Edit</MenuItem>}
        {canDeleteProject && (
