...
export const getAllSubcontrolsQuery = async (organizationId: number): Promise<ISubcontrol[]> => {
  ...
  const setClause = [
    ...
    .filter((f) => {
      if (
        ...
        updateSubControl[f as keyof SubcontrolModel] = subcontrol[f as keyof SubcontrolModel];
        return true;
      }
      return false;
    })
