...
const markEnqueued = jest.fn();
const runScheduledReport = jest.fn();
jest.mock("../../../utils/scheduledReport.utils", () => ({
  findDueScheduledReportsQuery: (...a: any) => findDue(...a),
  markRunEnqueuedQuery: (...a: any) => markEnqueued(...a),
}));
jest.mock("../reportRunOrchestrator", () => ({
  runScheduledReport: (...a: any) => runScheduledReport(...a),
}));
...
describe("handleReportSchedulerTick", () => {
  ...
  it("runs each due report and advances next_run", async () => {
    findDue.mockResolvedValue([
      { id: 3, schedule_config: { frequency: "daily", hour: 9, minute: 0, timezone: "UTC" } },
    ]);
