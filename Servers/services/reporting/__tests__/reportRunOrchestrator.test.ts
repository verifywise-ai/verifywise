...
jest.mock("../reportDeliveryService", () => ({
  deliverReport: (...a: any) => deliverReport(...a),
}));
jest.mock("../../../utils/reportRun.utils", () => ({
  createRunQuery: (...a: any) => createRunQuery(...a),
  updateRunStatusQuery: (...a: any) => updateRunStatusQuery(...a),
}));
...
const sched = {
  ...
  scope: "project",
  project_id: 5,
  sections_config: { sections: [{ reportSectionKey: "projectRisks", defaultEnabled: true }] },
  ai_blocks_config: { executiveSummary: true },
  delivery_config: { saveToStorage: true, sendEmailLink: false, attachFile: false, recipients: [] },
};

describe("runScheduledReport", () => {
  beforeEach(() => {
    [generateReport, deliverReport, createRunQuery, updateRunStatusQuery].forEach((m) =>
      m.mockReset(),
    );
  ...
  it("success path: generate + deliver -> success", async () => {
    ...
    deliverReport.mockResolvedValue({
      storage: { status: "success", fileId: 5 },
      emailLink: { status: "skipped" },
      attachment: { status: "skipped" },
      fileId: 5,
    });
    const outcome = await runScheduledReport(sched as any, {
  ...
  // framework_ids co-decides which projects_frameworks pairings the report
  // covered. Without it in the snapshot, two runs of one schedule either side
  // of a PATCH that changed frameworkIds look identical in report_runs.
  it("snapshots the framework selection the run actually used", async () => {
    generateReport.mockResolvedValue({
      success: true,
      content: Buffer.from("x"),
      filename: "r.pdf",
    ...
    await runScheduledReport({ ...sched, framework_ids: ["native:2", "native:3"] } as any, {
      triggeredBy: "scheduler",
      scheduledFor: new Date(),
    });
    expect(createRunQuery).toHaveBeenCalledWith(
      expect.objectContaining({
        config_snapshot: expect.objectContaining({ framework_ids: ["native:2", "native:3"] }),
      }),
    );
  });

  // NULL is the schedule's own "every framework in scope" — record it as null
  // rather than omitting the key, so the run says "all" instead of "unknown".
  it("snapshots a null framework selection as null", async () => {
    generateReport.mockResolvedValue({
      success: true,
      content: Buffer.from("x"),
      filename: "r.pdf",
    ...
    await runScheduledReport(sched as any, { triggeredBy: "scheduler", scheduledFor: new Date() });
    const snapshot = createRunQuery.mock.calls[0][0].config_snapshot;
    expect(snapshot).toHaveProperty("framework_ids");
    expect(snapshot.framework_ids).toBeNull();
  });
  ...
  it("snapshots a null project for an organization-scoped run", async () => {
    ...
    await runScheduledReport({ ...sched, project_id: null } as any, {
      triggeredBy: "manual",
      userId: 3,
    });
