...
- **Cross-tenant edge.** Owner org and attacker org each own one risk, and both risks get `controls_eu__risks` rows naming the *same three* `control_id`s, each row with its own `organization_id`. No shared category, so tier 0 contributes nothing. Assert first that `getStructuralNeighboursQuery` itself returns `[]`, then that a recompute writes no row. **The query-level assertion is the load-bearing one:** the merge guard above would keep `risk_links` empty even if the SQL leaked, so the end-to-end assertion alone cannot fail on an SQL mutation. Note also that the two filter layers are belt and braces — each alone suffices — so the leak surfaces only with *both* the arm filter and the `risks`-join filter removed; with both gone each control reaches degree 2 and tier 1 scores 3.79.
- **Degree scoping.** Subject and one partner risk in the owner org, both on the *same three* controls. Also seed, on those same controls, (a) five risks in a second org, (b) one **soft-deleted** owner-org risk, and (c) an **orphan** join row: `organization_id` = owner, `projects_risks_id` = 999999, naming no risk at all. Recompute the subject and assert the stored `score` is **3.79** — three elements at the owner org's own live degree of 2.

  Two *single-edit* mutations turn this red while leaving the cross-tenant case green, and the orphan row is what keeps them from being the same observation twice. Computing `degrees` `FROM element_links` instead of `FROM active` counts the soft-deleted risk *and* the orphan: degree 4, score 2.58, under the threshold, so no row is written. Dropping `AND r.is_deleted = false` counts only the soft-deleted risk, because the join to `risks` still discards the orphan: degree 3, a row stored at 3.00. Without the orphan both mutations give degree 3 and the test would pin one property while appearing to pin two. The second org's five risks cannot enter under either mutation; they are seeded to pin the property end to end, and with both org filters removed the degree becomes 8, the score 1.89, and again no row.
...
The query is a ten-table `UNION ALL` filtered by `organization_id`, and **eight of those ten tables have no index on `organization_id`** — checked, not assumed. Only `custom_framework_level2_risks` and `custom_framework_level3_risks` carry one (`idx_cf_l2_risks_org`, `idx_cf_l3_risks_org`); the rest have a primary key on `(element_id, risk_id)` and nothing else. Postgres will sequentially scan them.

That is still acceptable to ship, and A2a adds no index. These are narrow join tables — three integer columns, one row per attachment — and the scan is the same shape A1 already accepted, where every job scans `risks` and `projects_risks`. The honest limit is the backfill: N jobs each scanning ten tables, so cost grows with (risks in the org) × (attachments across *all* orgs). An org with thousands of risks will feel that before a single interactive save ever does.

The fix, when it is measured rather than imagined, is one index per table on `(organization_id, projects_risks_id)`. It needs a migration, which A2a explicitly excludes, and the seam is `getStructuralNeighboursQuery` — nothing in the provider changes. Do not add it in this phase.
