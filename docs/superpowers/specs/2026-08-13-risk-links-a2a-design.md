...
- **Degree scoping.** Subject and one partner risk in the owner org, both on the *same three* controls. Also seed (a) five risks in a second org on those controls and (b) one **soft-deleted** owner-org risk on them. Recompute the subject and assert the stored `score` is **3.79** — three elements at the owner org's own live degree of 2. Two *single-edit* mutations turn this red and leave the cross-tenant case green: computing `degrees` `FROM element_links` instead of `FROM active`, or dropping `AND r.is_deleted = false`. Both give degree 3 and a stored 3.00. The second org's five risks cannot enter under either of those; they are seeded to pin the property end to end, and with both org filters removed the degree becomes 8, the score 1.89, and no row is written at all.
