...
- ~~**`updateRunStatusQuery` has no `organization_id` in its WHERE clause** and hardcodes `completed_at = NOW()`.~~ **Closed.** The signature is now `(id, organization_id, fields)`, the WHERE clause is org-scoped, and `completed_at` is stamped only for the terminal statuses (`success`, `failed`, `partial_success`). All callers — `reporting.ctrl.ts`, `manualReportRunner.ts` (×4), `reportRunOrchestrator.ts` (×3) — pass the org id they already had in scope. Closed alongside it: `updateTemplate` ran its metadata UPDATE and its version INSERT as two autocommitted statements, so a failing version insert (e.g. the 23505 version race) left the metadata committed; `updateTemplateQuery` now takes an optional `transaction` and the controller wraps both writes in one `sequelize.transaction()`, matching `createTemplate`. The two 404 paths throw `NotFoundException` rather than returning, so they roll back.
